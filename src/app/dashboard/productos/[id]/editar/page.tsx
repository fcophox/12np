...
export default function EditarProductoPage() {
  ...
  useEffect(() => {
    ...
    if (p) {
      ...
      setOrden(p.orden);
      setPrecio(p.precio != null ? p.precio.toLocaleString("es-CL") : "");
      setDestacado(p.destacado);
      setImagen(p.imagen);
      setCreadoEn(p.creadoEn);
      setPausado(p.pausado ?? false);
      setLoaded(true);
    }
  }, [productos, id, loaded]);
  ...
  const eliminarEtiqueta = (tag: string) => setEtiquetas((prev) => prev.filter((t) => t !== tag));
  ...
  return (
    <div className="p-10 max-w-2xl">
      ...
      <div className="flex items-center gap-1.5 text-sm text-[#3d332e]/40 mb-6">
        ...
        <ChevronRight size={13} />
        <span className="text-[#3d332e]/70">Editar producto</span>
      </div>
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {imagen && (
            <button onClick={() => setImagen(null)} className="text-xs text-[#3d332e]/40 hover:text-[#f15a24] transition-colors self-start">
              Quitar imagen
            </button>
          )}
        </div>
        ...
        {/* Nombre */}
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium text-[#3d332e]">Nombre <span className="text-[#f15a24]">*</span></label>
          <input type="text" value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Ej: Café de especialidad"
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input type="text" value={frase} onChange={(e) => setFrase(e.target.value)} placeholder="Ej: El mejor café de la ciudad"
            className="w-full px-4 py-3 rounded-lg border border-[#e8e3dd] bg-white text-[#3d332e] text-sm placeholder:text-[#3d332e]/30 focus:outline-none focus:ring-2 focus:ring-[#f15a24]/20 focus:border-[#f15a24] transition" />
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input type="text" value={etiqueta} onChange={(e) => setEtiqueta(e.target.value)} placeholder="Ej: Nuevo, Popular, Temporada..."
            className="w-full px-4 py-3 rounded-lg border border-[#e8e3dd] bg-white text-[#3d332e] text-sm placeholder:text-[#3d332e]/30 focus:outline-none focus:ring-2 focus:ring-[#f15a24]/20 focus:border-[#f15a24] transition" />
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div onClick={() => etiquetaInputRef.current?.focus()}
            ...
            {etiquetas.map((tag) => (
              <span key={tag} className="flex items-center gap-1 bg-[#f9f4e8] border border-[#e8e3dd] text-[#3d332e] text-xs font-medium px-2.5 py-1 rounded-full">
                {tag}
                <button type="button" onClick={(e) => { e.stopPropagation(); eliminarEtiqueta(tag); }}
                  className="text-[#3d332e]/30 hover:text-[#f15a24] transition-colors leading-none cursor-pointer">×</button>
              </span>
            ))}
            <input ref={etiquetaInputRef} type="text" value={etiquetaInput}
        ...
        <div className="flex gap-4">
          <div className="flex flex-col gap-1.5 flex-1">
            ...
            <input type="number" min={1} value={orden} onChange={(e) => setOrden(Number(e.target.value))}
          ...
          <div className="flex flex-col gap-1.5 flex-1">
            ...
            <div className="flex items-center gap-3 py-1">
              <button type="button" onClick={() => setDestacado(!destacado)}
                className={`relative rounded-full transition-colors cursor-pointer shrink-0 ${destacado ? "bg-[#f15a24]" : "bg-[#e8e3dd]"}`}
                style={{ height: "22px", width: "40px" }}>
                <span className={`absolute top-0.5 left-0.5 w-[18px] h-[18px] rounded-full bg-white shadow transition-transform ${destacado ? "translate-x-[18px]" : "translate-x-0"}`} />
              </button>
              <span className="text-sm text-[#3d332e]/60">{destacado ? "Sí, destacado" : "No destacado"}</span>
            </div>
